...
const DashboardBarbero = () => {
  ...
  useEffect(() => {
    reservasAPI.getAll({
      ...
      barbero_id: user?.usuario_id,
    })
      .then(({ data }) => setAgenda(data.data))
      .catch((e) => console.error('Dashboard barbero error:', e))
      .finally(() => setLoading(false))
  }, [])
  ...
  const completadas = agenda.filter(r => r.estado === 'completada').length
  const pendientes  = agenda.filter(r => r.estado === 'pendiente' || r.estado === 'confirmada').length
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Mis citas hoy"  value={total}       color="amber" />
        <StatCard label="Completadas"    value={completadas} color="green" />
        <StatCard label="Pendientes"     value={pendientes}  color="blue" />
...
const Dashboard = () => {
  ...
  return rol === 'admin' ? <DashboardAdmin /> : <DashboardBarbero />
}
